...
class MetaLeadsController {
  ...
  static async getFolderMappings(req, res) {
    try {
      const folderMappings = await MetaLeadFolderMapping.find()
        .populate('customerId', 'name email')
        .sort({ folderName: 1 });
  ...
  static async createFolderMapping(req, res) {
    try {
      ...
      if (!customerId || !folderName || !String(folderName).trim()) {
      ...
      const existing = await MetaLeadFolderMapping.findOne({ folderNameLower: folderLower });
      if (existing) {
        return res.status(400).json({ message: 'This folder is already mapped to a clinic. One folder can only be assigned to one clinic.' });
      }

      const mapping = new MetaLeadFolderMapping({
        customerId,
        folderName: name,
        folderNameLower: name.toLowerCase(),
        notes: notes || null
      });
  ...
  static async updateFolderMapping(req, res) {
    try {
      ...
      if (folderName !== undefined && String(folderName).trim()) {
        const newName = String(folderName).trim();
        const newLower = newName.toLowerCase();
        if (newLower !== (mapping.folderNameLower || '')) {
          const existing = await MetaLeadFolderMapping.findOne({ folderNameLower: newLower, _id: { $ne: mappingId } });
          if (existing) {
            return res.status(400).json({ message: 'This folder name is already mapped to another clinic.' });
          }
        }
        mapping.folderName = newName;
        mapping.folderNameLower = newLower;
      }
