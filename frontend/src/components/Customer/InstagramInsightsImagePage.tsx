...
const InstagramInsightsImagePage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-6">
      ...
      {/* Right: Image Display */}
      <div className="w-full lg:w-2/3 flex flex-col items-center justify-center min-h-[60vh] px-2 py-8 bg-gradient-to-br from-[#f8fafc] to-[#fce4ec]">
        <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl p-10 flex flex-col items-center border border-[#fce4ec] relative">
          {selectedImage ? (
            <>
              <h1 className="text-3xl font-extrabold text-pink-600 mb-2 tracking-tight text-center font-sans">
                {getMonthLabel(selectedImage.month)} Instagram Insights
              </h1>
              <img
                src={BACKEND_BASE_URL + selectedImage.imageUrl}
                alt="Instagram Insights"
                className="max-w-full max-h-96 rounded shadow mb-4"
              />
